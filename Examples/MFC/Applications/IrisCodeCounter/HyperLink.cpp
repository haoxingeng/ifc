...
CHyperLink::CHyperLink()
{
    ...
    m_crHoverColor     = RGB(255,  92,   0);   //::GetSysColor(COLOR_3DHILIGHT);
...
void CHyperLink::SetUrl(LPCTSTR lpszUrl)
{
    ...
    {
        PositionWindow();
        m_ToolTip.UpdateTipText(lpszUrl, this, TOOLTIP_ID);
    }
}
...
HINSTANCE CHyperLink::GotoUrl(LPCTSTR lpszUrl, int nShowCmd)
{
    TCHAR key[MAX_PATH + MAX_PATH];

    // First try ShellExecute()
    HINSTANCE result = ShellExecute(NULL, _T("open"), lpszUrl, NULL,NULL, nShowCmd);

    // If it failed, get the .htm regkey and lookup the program
    if ((UINT)result <= HINSTANCE_ERROR) 
    {
        struct {
            LONG operator() (HKEY key, LPCTSTR subkey, LPTSTR retdata)
            {
                HKEY hkey;
...
#ifdef _UNICODE
                USES_CONVERSION;
                result = (HINSTANCE) WinExec(T2A(key),nShowCmd);
#else
