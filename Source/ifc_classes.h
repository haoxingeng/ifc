...
public:
    ...
    /// Get the byte value as const char by index(0-based).
    const char& operator [] (int nIndex) const { return ((char*)m_pBuffer)[nIndex]; }

    /// Get the byte value as char by index(0-based).
    ...
    /// Return the buffer pointer as a regular C string, the return string is null-terminated.
    ///
    /// @remarks
...
///////////////////////////////////////////////////////////////////////////////
/// CStream <b>[Delphi::TStream]</b> - The base class for stream.
///
...
public:
    ...
    /// @remarks
    ///   Use ReadBuffer to read @a nCount bytes from the stream into a buffer in cases where the 
    ///   number of bytes is known and fixed, for example when reading in structures.
    ///   ReadBuffer calls Read() to do the actual reading. If @a nCount bytes cannot be read from 
    ///   the stream, a CIfcStreamException exception is thrown.
    void ReadBuffer(void *pBuffer, int nCount);

    /// Writes @a nCount bytes from @a pBuffer onto the stream and advances the current position of the 
    /// stream by @a nCount bytes.
    ///
    /// @param[in] pBuffer
    ///   Pointer to the data to be written.
    /// @param[in] nCount
    ...
    void WriteBuffer(void *pBuffer, int nCount);
    ...
    /// @remarks
    ///   If @a nCount is 0, CopyFrom sets Source position to 0 before reading and then copies the 
    ///   entire contents of Source into the stream. If @a nCount is greater than or less than 0, 
...
///////////////////////////////////////////////////////////////////////////////
/// CCustomMemoryStream <b>[Delphi::TCustomMemoryStream]</b> - The base class for memory stream.
///
...
public:
    ...
    /// Reads up to @a nCount bytes from the memory stream into @a pBuffer and advances the current position 
...
///////////////////////////////////////////////////////////////////////////////
/// CMemoryStream <b>[Delphi::TMemoryStream]</b> - Memory stream class.
///
...
public:
    ...
    /// Writes @a nCount bytes from @a pBuffer to the current position in the memory buffer and 
    /// updates the current position by @a nCount bytes.
    ///
    /// Use Write to insert @a nCount bytes into the memory buffer of the memory stream, starting at 
    ...
    /// Write always writes the @a nCount bytes in the @a pBuffer, unless there is a memory failure. 
...
///////////////////////////////////////////////////////////////////////////////
/// CFileStream <b>[Delphi::TFileStream]</b> - File stream class.
///
...
private:
    void Init();
    HANDLE FileCreate(LPCTSTR lpszFileName, DWORD nRights);
...
public:
    ...
    /// Reads up to @a nCount bytes of data from the file stream into @a pBuffer. 
    ///
    /// @param[in] pBuffer
    ///   Storge the data read from stream.
    /// @param[in] nCount
    ...
    /// Writes @a nCount bytes from the @a pBuffer to the current position in the file.
    /// 
    /// @param[in] pBuffer
    ///   Pointer to the data to be written.
    /// @param[in] nCount
...
///////////////////////////////////////////////////////////////////////////////
/// CResourceStream <b>[Delphi::TResourceStream]</b> - Resource stream class.
///
...
private:
    ...
    HANDLE m_hGlobal;
private:
    void Initialize(HINSTANCE hInstance, LPVOID pResName, LPCTSTR lpszResType, bool bIsFromId);
public:
...
///////////////////////////////////////////////////////////////////////////////
/// CSeqAllocator - Sequence number allocator class.
///
/// @remarks
///   @li CSeqAllocator is thread-safe.
...
public:
    ...
    explicit CSeqAllocator(DWORD nStartId = 0);
...
///////////////////////////////////////////////////////////////////////////////
/// CPointerList <b>[Delphi::TList]</b> - Pointer list class.

class CPointerList
{
public:
    /// The comparison function prototype that used by Sort().
    typedef int (*LIST_COMPARE_PROC)(LPVOID pItem1, LPVOID pItem2);
private:
    LPVOID *m_pList;
    int m_nCount;
    int m_nCapacity;
protected:
    ...
    void Put(int nIndex, LPVOID pItem);
    void SetCount(int nNewCount);
    void QuickSort(LPVOID *pList, int l, int r, LIST_COMPARE_PROC pfnCompareProc);
...
public:
    ...
    void Insert(int nIndex, LPVOID pItem);
    ...
    ///   - (-1), if not found.
    int IndexOf(LPVOID pItem) const;
    ...
    const LPVOID& operator [] (int nIndex) const;
...
///////////////////////////////////////////////////////////////////////////////
/// CStrings <b>[Delphi::CStrings]</b> - The abstract base class for string list.
///
...
public:
    ...
    /// @return
    ///   The index (0-based) of the new string.
    virtual int Add(LPCTSTR lpszStr, LPVOID pData);
    ...
    virtual void Insert(int nIndex, LPCTSTR lpszStr, LPVOID pData);
    ...
    ///   - The index of the first occurrence in the list, or
    ///   - (-1), if not found.
    virtual int IndexOfData(LPVOID pData) const;
    ...
    virtual void LoadFromStream(CStream& Stream, bool bAsUnicode);
    ...
    virtual void LoadFromFile(LPCTSTR lpszFileName, bool bAsUnicode);
    ...
    virtual void SaveToStream(CStream& Stream, bool bAsUnicode) const;
    ...
    virtual void SaveToFile(LPCTSTR lpszFileName, bool bAsUnicode) const;
    ...
    virtual LPVOID GetData(int nIndex) const { return NULL; }
    ...
    virtual void SetData(int nIndex, LPVOID pData) {}
...
///////////////////////////////////////////////////////////////////////////////
/// CStrList <b>[Delphi::TStringList]</b> - String list class.
...
protected: // virtual
    ...
    virtual void InsertItem(int nIndex, LPCTSTR lpszStr, LPVOID pData);
public:
    CStrList();
    ...
    /// Returns the data pointer associated with the string at the specified position.
    virtual LPVOID GetData(int nIndex) const;
...
///////////////////////////////////////////////////////////////////////////////
/// CHashedStrList <b>[Delphi::THashedStringList]</b> - Hashed string list class.
///
...
///////////////////////////////////////////////////////////////////////////////
/// CRegistry <b>[Delphi::TRegistry]</b> - Registry class.
///
...
public:
    ...
    ///   throws an exception. 
    int ReadBinaryData(LPCTSTR lpszName, LPVOID pBuffer, int nBufSize);
    ...
    void WriteBinaryData(LPCTSTR lpszName, LPVOID pBuffer, int nBufSize);
...
///////////////////////////////////////////////////////////////////////////////
/// CCustomIniFile <b>[Delphi::TCustomIniFile]</b> - The base class for manipulating INI file.
///
...
public:
    ...
    void WriteBinaryData(LPCTSTR lpszSection, LPCTSTR lpszName, LPVOID pDataBuf, int nDataSize);
...
///////////////////////////////////////////////////////////////////////////////
/// CIniFile <b>[Delphi::TIniFile]</b> - INI file class.
///
...
///////////////////////////////////////////////////////////////////////////////
/// CMonitor <b>[Delphi::CMonitor]</b> - Monitor class.
///
...
///////////////////////////////////////////////////////////////////////////////
/// CScreen <b>[Delphi::TScreen]</b> - Screen class.
///
...
{
private:
    static std::auto_ptr<CScreen> m_pSingleton;
private:
...
///////////////////////////////////////////////////////////////////////////////
/// CClipboard <b>[Delphi::TClipboard]</b> - Clipboard class.
///
...
private:
    static std::auto_ptr<CClipboard> m_pSingleton;
private:
    int m_nOpenRefCount;
    bool m_bEmptied;
    HWND m_hWndOwner;
private:
    HWND GetOwnerWindow();
    void BeforeSetData();
    void SetBuffer(int nFormat, LPVOID pBuffer, int nSize);
protected:
    CClipboard();
public:
    ~CClipboard() {}
